...
function getMean(arr, x, y) {
    let firstArr = arr.slice(0, x)
    let secondArr = arr.slice(-y)
    
    if((x <= 1 || y<= 1) || (y > arr.length || x > arr.length)){
      return -1
    }
    else{
    let firstSum =firstArr.reduce((a,b)=> a+b)/firstArr.length
    let SecondSum = secondArr.reduce((a,b)=> a+b)/secondArr.length
