...
function reverseNumber(n, b) {
    ...
    function translateToBinaryAndReverse(number) {
        while (number > 0n) {
            reverseArr.push(number % b)
            number = BigInt(Math.floor(Number(number) / Number(b)))           
        }
    }

    translateToBinaryAndReverse(n)

    rank =  BigInt(reverseArr.length) - 1n
    console.log(rank);

    reverseArr.forEach(digit => {    
        rank > 0n ? finArr.push(Number(digit) * (Math.pow(Number(b),Number(rank)))) : finArr.push(Number(digit))
        rank--
    })
    
    return BigInt(finArr.reduce((a, b) => a + b))
